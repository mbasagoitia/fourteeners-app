...
const NavDrawer = ({ user, setUser, navigate, handleDrawerClose }) => {
  
    const NavLink = ({to, title}) => {
      return <Nav.Link className="expNavLink" kas={Link} to={to} onClick={handleDrawerClose}>{title}</Nav.Link>
    }
    ...
    return (
      <div className="nav-drawer w-100" onMouseLeave={handleDrawerClose}>
        <div className="nav-drawer-content w-100 d-flex justify-content-evenly">
          ...
          <div>
              ...
              <NavLink to="/provide-feedback" title="Rate/Review the Tool" />
          </div>
          <div>
              ...
              <NavLink to="/browse-all-peaks" title="Browse All Peaks" />
          </div>
          <div>
          {user ? (
          <>
